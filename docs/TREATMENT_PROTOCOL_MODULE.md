...
9	GfrRuleApplier	                 Почечная фильтрация (GFR)	               Поддерживает форматы Class B - 12h или <60 ml/min - avoid. Может уменьшить дозу, изменить интервал или исключить препарат при низком GFR.
10	WeightRuleApplier	             Вес пациента	                           Если вес < 50 кг — корректирует дозу (mg) или интервал приёма (h) по данным протокола.
