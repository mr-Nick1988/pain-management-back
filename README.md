...
унификация тире в обычный -  , тримминг и схлопывание множественных пробелов, приведение «служебных» значений к null (NA, N/A, —, пусто).
для route — мягкое сопоставление с enum (case-insensitive + словарь алиасов: oral, p/o → PO, и т. п.);
улучшили парсинг диапазона боли в TreatmentProtocolService: вытаскиваем только цифры и один дефис, не падаем на мусоре;
...
Кроме того, часть сущностей хранилась в common-пакете, но не успевала подхватываться, если Hibernate не пересоздавал схему.
Исправление:
...
- Это про MongoDB! Если добавите новое поле в [AnalyticsEvent], не нужно менять схему БД (как в PostgreSQL с ALTER TABLE)
