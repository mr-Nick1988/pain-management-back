...
public class PainEscalationServiceImpl implements PainEscalationService {
    ...
    public PainEscalationCheckResult checkPainEscalation(String mrn) {
        ...
        PainTrendAnalysis trendAnalysis = analyzePainTrend(mrn);
        ...
        return PainEscalationCheckResult.builder()
                .patientMrn(mrn)
                ...
                .painTrendAnalysis(trendAnalysis)
                .build();
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public PainTrendAnalysis analyzePainTrend(String mrn) {
        Patient patient = patientRepository.findByMrn(mrn)
                .orElseThrow(() -> new NotFoundException("Patient with MRN " + mrn + " not found"));

        List<Vas> vasHistory = patient.getVas();
        if (vasHistory == null || vasHistory.isEmpty()) {
            return PainTrendAnalysis.builder()
                    .patientMrn(mrn)
                    .painTrend("UNKNOWN")
                    .vasRecordCount(0)
                    .build();
    ...
    public void handleNewVasRecord(String mrn, Integer newVasLevel) {
        ...
        PainEscalationCheckResult checkResult = checkPainEscalation(mrn);
     ...
     */
    private PainEscalationCheckResult buildNoEscalationResult(String mrn, String reason) {
        return PainEscalationCheckResult.builder()
                .patientMrn(mrn)
    ...
    @Override
    @Transactional(readOnly = true)
    public PainEscalationCheckResult checkPainEscalation(String mrn, PainEscalationController.PainEscalationCheckCommand command) {
    ...
    public DoseAdministrationResponseDTO registerDoseAdministration(String mrn, DoseAdministrationRequestDTO request) {
        ...
        return DoseAdministrationResponseDTO.builder()
                .id(saved.getId())
                .patientMrn(mrn)
                .recommendationId(saved.getRecommendation() != null ? saved.getRecommendation().getId() : null)
                .drugName(saved.getDrugName())
                .dosage(saved.getDosage())
                .route(saved.getRoute())
                .administeredBy(saved.getAdministeredBy())
                .vasBefore(saved.getVasBefore())
                .vasAfter(saved.getVasAfter())
